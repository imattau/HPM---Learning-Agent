"""Experimental interconnected-polygraph SNIPS benchmark for HPM v5."""
...
from hpm_ai_v5.adapter.snips import IntentLabelAdapter, SNIPSBridgeAnchorAdapter, SNIPSCanonicalIntentAdapter, load_snips
...
class InterconnectedSNIPSBenchmark:
    """SNIPS benchmark with bridge-aware multi-view aggregation."""
    ...
    def __init__(self, consolidation: bool = True):
        ...
        self.pipeline, self.intent_adapter = build_intent_pipeline(
            ...
            bridge_adapter=SNIPSBridgeAnchorAdapter(),
    ...
    def _predict(self, text: str, *, use_retriever: bool = True, use_concept_scoring: bool = False) -> tuple[str | None, dict]:
        if self._inference_agent is None:
            raise RuntimeError("train() must be called before predict()")
        packet = AgentPacket(raw=text)
        self._inference_agent.step_packet(packet, use_retriever=use_retriever, use_concept_scoring=use_concept_scoring)
        return packet.final_output, packet.context

    @staticmethod
    def _concept_family(concept_id: str) -> str:
    ...
    def run_b1(self, train: list[dict], test: list[dict]) -> float:
        print("\nB1x: Interconnected SNIPS Intent Recognition...")
        self.train(train)
        correct = 0
        for item in test:
    ...
    def run_store_map(self) -> dict[str, float]:
    ...
    def run_b_retriever_comparison(self, test: list[dict]) -> dict[str, float]:
        ...
        anchor_plus_concept_correct = 0
        changed_prediction_count = 0
        concept_changed_prediction_count = 0
        improved_count = 0
        concept_improved_count = 0
        degraded_count = 0
        concept_degraded_count = 0
        concept_supported_candidate_count = 0
        total_concept_agreements = 0
        total_candidates = 0
        concept_family_breakdown: Counter[str] = Counter()

        for item in test:
            _, anchor_ctx = self._predict(item["text"], use_retriever=False)
            retriever_pred, retriever_ctx = self._predict(item["text"], use_retriever=True, use_concept_scoring=False)
            concept_pred, concept_ctx = self._predict(item["text"], use_retriever=True, use_concept_scoring=True)
            ...
            if concept_pred == item["intent"]:
                anchor_plus_concept_correct += 1
            if retriever_ctx.get("retriever_changed_prediction"):
                ...
                if anchor_pred != item["intent"] and retriever_pred == item["intent"]:
                    ...
                    degraded_count += 1
            if concept_ctx.get("retriever_changed_prediction"):
                concept_changed_prediction_count += 1
                if anchor_pred != item["intent"] and concept_pred == item["intent"]:
                    concept_improved_count += 1
                elif anchor_pred == item["intent"] and concept_pred != item["intent"]:
                    concept_degraded_count += 1
            concept_candidates = concept_ctx.get("polygraph_candidates", ())
            total_candidates += len(concept_candidates)
            concept_supported_candidate_count += sum(1 for candidate in concept_candidates if candidate.get("concept_support"))
            for candidate in concept_candidates:
                concept_support = candidate.get("concept_support", {})
                total_concept_agreements += len(concept_support)
                if concept_ctx.get("retriever_changed_prediction") and concept_support:
                    for concept_id in concept_support:
                        concept_family_breakdown[self._concept_family(concept_id)] += 1

        total = max(len(test), 1)
        candidate_total = max(total_candidates, 1)
        metrics = {
            "anchor_only_accuracy": anchor_only_correct / total,
            "anchor_plus_retriever_accuracy": anchor_plus_retriever_correct / total,
            "anchor_plus_concept_accuracy": anchor_plus_concept_correct / total,
            "changed_prediction_rate": changed_prediction_count / total,
            "concept_changed_prediction_rate": concept_changed_prediction_count / total,
            "improved_rate": improved_count / total,
            "concept_improved_rate": concept_improved_count / total,
            "degraded_rate": degraded_count / total,
            "concept_degraded_rate": concept_degraded_count / total,
            "concept_supported_candidate_count": concept_supported_candidate_count / total,
            "avg_concept_agreements_per_candidate": total_concept_agreements / candidate_total,
            "concept_family_breakdown": dict(sorted(concept_family_breakdown.items())),
        }
        ...
        print(f"  Anchor+concept accuracy:     {metrics['anchor_plus_concept_accuracy']:.2%}")
        print(f"  Changed prediction rate:     {metrics['changed_prediction_rate']:.2%}")
        print(f"  Concept changed rate:        {metrics['concept_changed_prediction_rate']:.2%}")
        print(f"  Improved prediction rate:    {metrics['improved_rate']:.2%}")
        print(f"  Concept improved rate:       {metrics['concept_improved_rate']:.2%}")
        print(f"  Degraded prediction rate:    {metrics['degraded_rate']:.2%}")
        print(f"  Concept degraded rate:       {metrics['concept_degraded_rate']:.2%}")
        print(f"  Concept-supported cand/u:    {metrics['concept_supported_candidate_count']:.2f}")
        print(f"  Avg concept agreements/cand: {metrics['avg_concept_agreements_per_candidate']:.2f}")
        if metrics["concept_family_breakdown"]:
            print(f"  Concept families:            {metrics['concept_family_breakdown']}")
        return metrics
    ...
    def run_all(self):
        ...
        print("\n" + "=" * 40)
        print("INTERCONNECTED SNIPS RESULTS")
        print("=" * 40)
        ...
        print(f"B4x Anchor+Concept Accuracy: {b4['anchor_plus_concept_accuracy']:.2%}")
        print(f"B4x Concept-supported Cand/Utt: {b4['concept_supported_candidate_count']:.2f}")
        print(f"B4x Avg Concept Agreements/Cand: {b4['avg_concept_agreements_per_candidate']:.2f}")
        print("=" * 40)


if __name__ == "__main__":
    random.seed(42)
    InterconnectedSNIPSBenchmark().run_all()
