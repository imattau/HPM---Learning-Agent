...
class SequenceDomain:
    ...
    def __init__(
        ...
        obs_seed, perturb_seed = ss.spawn(2)
        self._rng = np.random.default_rng(obs_seed)
        self._perturb_rng = np.random.default_rng(perturb_seed)
        self._transition = (
            transition
            if transition is not None
            else self._random_transition(self._rng, vocab_size)
        )
        self._label_map = (
            label_map
