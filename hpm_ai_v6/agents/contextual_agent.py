...
class ContextualAgent(SocialAgent):
    ...
    def rebuild_cells_from_patterns(self) -> None:
        for pattern in self.patterns:
            name = getattr(pattern, "name", "")
            if not name.startswith("ctx_") or "->" not in name:
                continue
            context_part, next_word = name.removeprefix("ctx_").split("->", 1)
            context_tokens = tuple(token for token in context_part.split("|") if token)
            if not context_tokens:
                continue
            token_cells = [self._get_or_create_word_cell(token) for token in context_tokens]
            context_cell = self.context_cells.get(context_tokens)
            if context_cell is None:
                embedding = np.mean([cell.as_numpy() for cell in token_cells], axis=0)
                context_cell = Cell(name=f"ctx_{'|'.join(context_tokens)}", dim=0, embedding=embedding)
                self.context_cells[context_tokens] = context_cell
            next_cell = self.word_cells.get(next_word) or Cell(
                name=f"word_{next_word}",
                dim=0,
                embedding=self._seed_embedding(next_word),
            )
            self.word_cells[next_word] = next_cell
            if pattern.source is None:
                pattern.source = context_cell
            if pattern.target is None:
                pattern.target = next_cell
