"""WordNet lexical cross-domain replay experiment.
...
introduces code-like observations before selectively replaying the most useful
seed nodes and finally revisiting the original seed stream:
  ...
  - code-like observations generated from Python token categories
  - selective replay of the seed observations tied to the highest-utility seed
    learned nodes
  - the original seed batch again, after the cross-domain interference

The goal is to test whether HFN/HPM can survive a domain switch and whether
...
from hpm_fractal_node.code.code_loader import generate_code_snippets
...
def _make_code_shift_batch(inventory, n_samples: int, seed: int) -> ObservationBatch:
    rng = np.random.default_rng(seed)
    raw = generate_code_snippets(seed=seed)
    indices = rng.choice(len(raw), size=n_samples, replace=True)
    items: list[tuple[str, str]] = []
    for obs_index, snippet_index in enumerate(indices):
        _, true_token, category = raw[snippet_index]
        category_seed = CODE_CATEGORY_SEEDS.get(category, "code")
        pool = CODE_CATEGORY_POOLS.get(category, CODE_CATEGORY_POOLS["data"])
        n_tokens = int(rng.integers(10, 16))
        tokens: list[str] = [category_seed, "code", "token"]
        if true_token.isalpha():
            cleaned = _normalize_token(true_token)
            if cleaned:
                tokens.append(cleaned)
        replace = len(pool) < max(n_tokens - len(tokens), 1)
        sampled = list(rng.choice(pool, size=max(n_tokens - len(tokens), 1), replace=replace))
        ...
        text = " ".join(deduped[:n_tokens])
        items.append((text, category))
    return _build_text_observation_batch(
        inventory,
        items,
        seed=seed,
        source_name="code_shift",
        relation_prefix="code",
    )
...
def _print_summary(
    ...
    selected_seed_remaining_after_code: int,
    ...
    code_result = result_by_name.get("code_shift")
    selective_result = result_by_name.get("selective_replay")
    revisit_result = result_by_name.get("revisit_seed")
    if seed_result and pressure_result and code_result and selective_result and revisit_result:
        ...
        print(f"  Selected seed nodes remaining after code shift: {selected_seed_remaining_after_code}")
        ...
        print(f"  Code stage learned explanations: {code_result.stats.learned_nodes_explained}")
...
def main() -> None:
    print("WordNet lexical cross-domain replay experiment")
    print(f"  D={D}, N_SAMPLES={N_SAMPLES}, N_PASSES={N_PASSES}, SEED={SEED}")
    print("  Observation stream stretches a WordNet forest, switches to code-like observations, then selectively replays seed nodes.")
    ...
    code_batch = _make_code_shift_batch(inventory, N_SAMPLES, seed=SEED + 4)
    ...
    print(f"  Generated {len(code_batch.observations)} code observations")
    ...
    selected_seed_remaining_after_code = 0
    ...
    for stage_name, difficulty, batch, action in [
        ...
        ("code_shift", "cross_domain", code_batch, None),
        ...
        if action == "apply" and not pressure_applied:
            ...
            selected_seed_remaining_after_code = sum(1 for node_id in selected_seed_nodes if forest.get(node_id) is not None)
    ...
    _print_summary(
        ...
        forest,
        selected_seed_nodes,
        selected_seed_remaining_after_code,
