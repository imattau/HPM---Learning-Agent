...
- Measures coverage, learned-node growth, category purity, abstraction usage, and RSS.
...
## Observed Results
- Large mode builds about `8,948` protected priors in the current run.
- The Peter Rabbit corpus is fully covered in smoke and full runs.
- Full-scale runs still produce `0` learned nodes surviving in the semantic-only setup, which means the ontology already explains the stream too well.
- Mean explaining layer stays low, around the synset / POS band, so the system is mostly matching existing structure rather than inventing new one.
...
- This is the best evidence so far that HFN can serve as a reusable semantic substrate rather than a transient pattern matcher.
...
- It does not yet show consolidation, transfer, or retention under harder input.

## Notes
- Uses diagonal sigma nodes.
- The WordNet data must be available locally through NLTK.
- Best interpreted as a capability baseline for the lexical experiments that follow.
