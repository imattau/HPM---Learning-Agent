"""WordNet lexical consolidation experiment.

This experiment reuses the WordNet-backed lexical-semantic prior forest and
pushes it through three progressive stages before revisiting the original seed
stream:
  ...
  - the original seed batch again, after the forest has been stretched

The goal is to test whether HFN/HPM can consolidate what it learned under
pressure: does the revisit stage reuse earlier learned structure, explain the
same material with deeper abstractions, and retain the nodes created during the
earlier stages?
"""
...
def _print_consolidation_summary(results: list[StageResult], forest: TieredForest) -> None:
    ...
    if seed_result and revisit_result:
        ...
        retained_seed_learned = sum(1 for node_id in seed_learned if forest.get(node_id) is not None)
        print("\n  Consolidation focus")
        print(f"  Seed learned nodes created: {len(seed_learned)}")
        print(f"  Seed learned nodes retained after revisit: {retained_seed_learned}")
...
def main() -> None:
    print("WordNet lexical consolidation experiment")
    print(f"  D={D}, N_SAMPLES={N_SAMPLES}, N_PASSES={N_PASSES}, SEED={SEED}")
    print("  Observation stream stretches a WordNet forest, then replays the seed stream to test consolidation.")
    ...
    stage_plan = [
        ("seed", "easy", seed_batch),
        ("stretch", "medium", stretch_batch),
        ("stretch_hard", "hard", hard_batch),
        ("revisit_seed", "replay", revisit_batch),
    ]

    results: list[StageResult] = []
    for stage_name, difficulty, batch in stage_plan:
        results.append(_run_stage(stage_name, difficulty, batch, forest, prior_ids, observer))

    _print_consolidation_summary(results, forest)
