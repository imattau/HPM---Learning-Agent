...
class FunctionalSpatialPriorLoader(HFNLoader):
    ...
    def build(self) -> list[HFN] | list[LoadItem]:
        ...
        sigma[local_i] = 10.0
        sigma[local_d] = 0.02
        nodes.append(self._item(self._make_node("prior_identity", mu, sigma), role="prior", protected=True))
        ...
        nodes.append(self._item(make_geo("prior_flip_v",   get_flip_delta(lambda g: np.flip(g, axis=0))), role="prior", protected=True))
        nodes.append(self._item(make_geo("prior_flip_h",   get_flip_delta(lambda g: np.flip(g, axis=1))), role="prior", protected=True))
        nodes.append(self._item(make_geo("prior_rot_90",   get_flip_delta(lambda g: np.rot90(g, k=-1))), role="prior", protected=True))
        nodes.append(self._item(make_geo("prior_rot_180",  get_flip_delta(lambda g: np.rot90(g, k=-2))), role="prior", protected=True))
        nodes.append(self._item(make_geo("prior_rot_270",  get_flip_delta(lambda g: np.rot90(g, k=-3))), role="prior", protected=True))
...
class FunctionalSymbolicPriorLoader(HFNLoader):
    ...
    def build(self) -> list[HFN] | list[LoadItem]:
        ...
        mu[19] = 1.0
        nodes.append(self._item(self._make_node("prior_identity_rule", mu, np.ones(self.dim) * 0.5), role="prior", protected=True))

        mu, _ = self._base_mu_sigma(default_sigma=0.5)
        mu[16] = 1.0
        mu[17] = 1.0
        nodes.append(self._item(self._make_node("prior_parity_even", mu, np.ones(self.dim) * 0.5), role="prior", protected=True))
...
def build_functional_spatial_priors(D: int = 1800) -> list[HFN]:
    ...
    return FunctionalSpatialPriorLoader(D).build_nodes()


def build_functional_symbolic_priors() -> list[HFN]:
    """Returns 30D symbolic/attribute prior HFNs."""
    return FunctionalSymbolicPriorLoader().build_nodes()
